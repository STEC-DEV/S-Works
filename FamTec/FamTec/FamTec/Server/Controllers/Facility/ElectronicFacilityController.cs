    ...
    public class ElectronicFacilityController : ControllerBase
    {
        ...
        public async Task<IActionResult> ImportElectronicFacilityForm([FromForm][Required] IFormFile files)
        {
            try
            {
                ...
                ResponseModel<bool> model = await ElectronicFacilityService.ImportElectronicFacilityService(files).ConfigureAwait(false);
                if (model is null)
                    return BadRequest();
        ...
        public async Task<IActionResult> AddFacility([FromForm] FacilityDTO dto, [FromForm] IFormFile? files)
        {
            try
            {
                ...
                ResponseModel<FacilityDTO>? model = await ElectronicFacilityService.AddElectronicFacilityService(dto, files).ConfigureAwait(false);
        ...
        public async Task<IActionResult> GetAllElecFacility()
        {
            try
            {
                ...
                CreateBuilderLogger.ConsoleText($"{HttpContext.Request.Path.Value}");
#endif
                ResponseModel<List<FacilityListDTO>>? model = await ElectronicFacilityService.GetElectronicFacilityListService().ConfigureAwait(false);
        ...
        public async Task<IActionResult> DetailElecFacility([FromQuery][Required] int facilityid)
        {
            try
            {
                ...
                ResponseModel<FacilityDetailDTO> model = await ElectronicFacilityService.GetElectronicDetailFacilityService(facilityid, isMobile).ConfigureAwait(false);
                if (model is null)
                    return BadRequest();
        ...
        public async Task<IActionResult> UpdateElecFacility([FromForm] FacilityDTO dto, [FromForm] IFormFile? files)
        {
            try
            {
                ...
                ResponseModel<bool?> model = await ElectronicFacilityService.UpdateElectronicFacilityService(dto, files).ConfigureAwait(false);
        ...
        public async Task<IActionResult> DeleteElecFacility([FromBody][Required] List<int> delIdx)
        {
            try
            {
                ...
                ResponseModel<bool?> model = await ElectronicFacilityService.DeleteElectronicFacilityService(delIdx).ConfigureAwait(false);
