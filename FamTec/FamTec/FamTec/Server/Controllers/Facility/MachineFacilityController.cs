    ...
    public class MachineFacilityController : ControllerBase
    {
        ...
        public async Task<IActionResult> ImportMachineFacilityForm([FromForm][Required] IFormFile files)
        {
            try
            {
                ...
                ResponseModel<bool> model = await MachineFacilityService.ImportMachineFacilityService(files);
                if (model is null)
                    return BadRequest();
        ...
        public async Task<IActionResult> AddFacility([FromForm] FacilityDTO dto, [FromForm]IFormFile? files)
        {
            try
            {
                ...
                ResponseModel<FacilityDTO> model = await MachineFacilityService.AddMachineFacilityService(dto, files).ConfigureAwait(false);
        ...
        public async Task<IActionResult> GetAllMachineFacility()
        {
            try
            {
                ...
                ResponseModel<List<FacilityListDTO>>? model = await MachineFacilityService.GetMachineFacilityListService().ConfigureAwait(false);
        ...
        public async Task<IActionResult> DetailMachineFacility([FromQuery][Required]int facilityid)
        {
            try
            {
                ...
                ResponseModel<FacilityDetailDTO> model = await MachineFacilityService.GetMachineDetailFacilityService(facilityid, isMobile).ConfigureAwait(false);
                if (model is null)
                    return BadRequest();
                if (model.code == 200)
                    return Ok(model);
        ...
        public async Task<IActionResult> UpdateMachineFacility([FromForm] FacilityDTO dto, [FromForm] IFormFile? files)
        {
            try
            {
                ...
                ResponseModel<bool?> model = await MachineFacilityService.UpdateMachineFacilityService(dto, files).ConfigureAwait(false);
                if (model is null)
                    return BadRequest();
                if (model.code == 200)
                    return Ok(model);
        ...
        public async Task<IActionResult> DeleteMachineFacility([FromBody][Required] List<int> delIdx)
        {
            try
            {
                ...
                ResponseModel<bool?> model = await MachineFacilityService.DeleteMachineFacilityService(delIdx).ConfigureAwait(false);
