    ...
    public class FireFacilityController : ControllerBase
    {
        ...
        public async Task<IActionResult> ImportFireFacilityForm([FromForm][Required] IFormFile files)
        {
            try
            {
                ...
                ResponseModel<bool> model = await FireFacilityService.ImportFireFacilityService(files).ConfigureAwait(false);
                if (model is null)
                    return BadRequest();
        ...
        public async Task<IActionResult> AddFacility([FromForm] FacilityDTO dto, [FromForm] IFormFile? files)
        {
            try
            {
                ...
                ResponseModel<FacilityDTO>? model = await FireFacilityService.AddFireFacilityService(dto, files).ConfigureAwait(false);
        ...
        public async Task<IActionResult> GetAllFireFacility()
        {
            try
            {
                ...
                CreateBuilderLogger.ConsoleText($"{HttpContext.Request.Path.Value}");
#endif
                ResponseModel<List<FacilityListDTO>>? model = await FireFacilityService.GetFireFacilityListService().ConfigureAwait(false);
        ...
        public async Task<IActionResult> DetailFireFacility([FromQuery][Required] int facilityid)
        {
            try
            {
                ...
                ResponseModel<FacilityDetailDTO> model = await FireFacilityService.GetFireDetailFacilityService(facilityid, isMobile).ConfigureAwait(false);
                if (model is null)
                    return BadRequest();
                if (model.code == 200)
                    return Ok(model);
        ...
        public async Task<IActionResult> UpdateFireFacility([FromForm] FacilityDTO dto, [FromForm] IFormFile? files)
        {
            try
            {
                ...
                ResponseModel<bool?> model = await FireFacilityService.UpdateFireFacilityService(dto, files).ConfigureAwait(false);
                if (model is null)
                    return BadRequest();
                if (model.code == 200)
                    return Ok(model);
        ...
        public async Task<IActionResult> DeleteFireFacility([FromBody][Required] List<int> delIdx)
        {
            try
            {
                ...
                ResponseModel<bool?> model = await FireFacilityService.DeleteFireFacilityService(delIdx).ConfigureAwait(false);
