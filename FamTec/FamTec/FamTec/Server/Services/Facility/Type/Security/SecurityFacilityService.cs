...
namespace FamTec.Server.Services.Facility.Type.Security
{
    public class SecurityFacilityService : ISecurityFacilityService
    {
        ...
        private readonly ConsoleLogService<SecurityFacilityService> CreateBuilderLogger; /* 콘솔로그 */
        private DirectoryInfo? di; /* 디렉터리 객체 */
        private string? SecurityFileFolderPath; /* 디렉터리 경로 */


        public SecurityFacilityService(IFacilityInfoRepository _facilityinforepository,
        ...
        /// <returns></returns>
        public async Task<byte[]?> DownloadSecurityFacilityForm()
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task<ResponseModel<bool>> ImportSecurityFacilityService(IFormFile? file)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
        ...
        public async Task<ResponseModel<FacilityDTO>> AddSecurityFacilityService(FacilityDTO dto, IFormFile? files)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                // 전기설비 관련한 폴더 없으면 만들기 
                SecurityFileFolderPath = Path.Combine(Common.FileServer, placeidx.ToString(), "Facility", "Security");
                ...
                if (result is not null)
                {
                    ...
                    {
                        // 파일 넣기
                        bool? AddFile = await FileService.AddResizeImageFile(NewFileName, SecurityFileFolderPath, files).ConfigureAwait(false);
                    }
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        public async Task<ResponseModel<List<FacilityListDTO>>> GetSecurityFacilityListService()
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                List<FacilityListDTO>? model = await FacilityInfoRepository.GetPlaceSecurityFacilityList(Convert.ToInt32(placeidx)).ConfigureAwait(false);
        ...
        public async Task<ResponseModel<FacilityDetailDTO>> GetSecurityDetailFacilityService(int facilityId, bool isMobile)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                if(isMobile)
                {
                    ...
                    {
                        byte[]? ImageBytes = await FileService.GetImageFile(SecurityFileFolderPath, model.Image).ConfigureAwait(false);
        ...
        public async Task<ResponseModel<bool?>> UpdateSecurityFacilityService(FacilityDTO dto, IFormFile? files)
        {
            try
            {
                // 파일처리 준비
                ...
                {
                    ImageBytes = await FileService.GetImageFile(SecurityFileFolderPath, deleteFileName).ConfigureAwait(false);
                }
                ...
                if (files is not null)
                {
                    if (String.IsNullOrWhiteSpace(model.Image) || files.FileName != model.Image)
                    {
                        ...
                        await FileService.AddResizeImageFile(model.Image!, SecurityFileFolderPath, files).ConfigureAwait(false);
                    }
                }
                ...
                else
                {
                    ...
                    if (AddTemp is not null)
                    {
                        try
                        {
                            ...
                            {
                                // 파일을 저장하는 로직
                                await FileService.AddResizeImageFile(AddTemp.FileName, SecurityFileFolderPath, files).ConfigureAwait(false);
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        public async Task<ResponseModel<bool?>> DeleteSecurityFacilityService(List<int> delIdx)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
