...
namespace FamTec.Server.Services.Facility.Type.Security
{
    public class SecurityFacilityService : ISecurityFacilityService
    {
        ...
        public async ValueTask<ResponseUnit<AddFacilityDTO>?> AddSecurityFacilityService(HttpContext? context, AddFacilityDTO? dto, IFormFile? files)
        {
            try
            {
                ...
                if (files is not null)
                {
                    ...
                    else
                    {
                        ...
                        string? newFilePath = Path.Combine(SecurityFileFolderPath, newFileName);
                ...
                if (model is not null)
                {
                    ...
                    if (room is not null)
                    {
                        ...
                        if (!String.IsNullOrWhiteSpace(Image))
                        {
                            ...
                            string[] FileList = Directory.GetFiles(SecurityFileFolderPath);
                            if (FileList is [_, ..])
                            {
        ...
        public async ValueTask<ResponseUnit<bool?>> UpdateSecurityFacilityService(HttpContext? context, UpdateFacilityDTO? dto, IFormFile? files)
        {
            try
            {
                string? FileName = String.Empty;
                ...
                if (model is not null)
                {
                    ...
                    if (files is not null)
                    {
                        ...
                        if (di.Exists)
                        {
                            ...
                            {
                                FileName = String.Format("{0}\\{1}", SecurityFileFolderPath, filePath);
        ...
        public async ValueTask<ResponseUnit<int?>> DeleteSecurityFacilityService(HttpContext? context, List<int> delIdx)
        {
            try
            {
                int delCount = 0;
