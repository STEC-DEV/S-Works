...
namespace FamTec.Server.Services.Facility.Type.Machine
{
    public class MachineFacilityService : IMachineFacilityService
    {
        ...
        private readonly ConsoleLogService<MachineFacilityService> CreateBuilderLogger; /* 콘솔로그 */
        private DirectoryInfo? di; /* 디렉터리 객체 */
        private string? MachineFileFolderPath; /* 디렉터리 경로 */

        public MachineFacilityService(IFacilityInfoRepository _facilityinforepository,
        ...
        /// <returns></returns>
        public async Task<ResponseModel<bool>> ImportMachineFacilityService(IFormFile? file)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
        ...
        /// <returns></returns>
        public async Task<ResponseModel<FacilityDTO>> AddMachineFacilityService(FacilityDTO dto, IFormFile? files)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        public async Task<ResponseModel<List<FacilityListDTO>>> GetMachineFacilityListService()
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                if (context is null)
                    return new ResponseModel<List<FacilityListDTO>>() { message = "요청이 잘못되었습니다.", data = new List<FacilityListDTO>(), code = 404 };

                string? placeidx = Convert.ToString(context.Items["PlaceIdx"]);
                if (String.IsNullOrWhiteSpace(placeidx))
                    return new ResponseModel<List<FacilityListDTO>>() { message = "요청이 잘못되었습니다.", data = new List<FacilityListDTO>(), code = 404 };

                List<FacilityListDTO>? model = await FacilityInfoRepository.GetPlaceMachineFacilityList(Convert.ToInt32(placeidx)).ConfigureAwait(false);
        ...
        public async Task<ResponseModel<FacilityDetailDTO>> GetMachineDetailFacilityService(int facilityId, bool isMobile)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                if(isMobile) 
                {
                    ...
                    {
                        byte[]? ImageBytes = await FileService.GetImageFile(MachineFileFolderPath, model.Image).ConfigureAwait(false);
        ...
        public async Task<ResponseModel<bool?>> UpdateMachineFacilityService(FacilityDTO dto, IFormFile? files)
        {
            try
            {
                // 파일처리 준비
                ...
                {
                    ImageBytes = await FileService.GetImageFile(MachineFileFolderPath, deleteFileName).ConfigureAwait(false);
                }
                ...
                if (!String.IsNullOrWhiteSpace(deleteFileName))
                {
                    FileService.DeleteImageFile(MachineFileFolderPath, deleteFileName);
                }
                ...
                if (files is not null)
                {
                    if (String.IsNullOrWhiteSpace(model.Image) || files.FileName != model.Image)
                    {
                        ...
                        await FileService.AddResizeImageFile(model.Image!, MachineFileFolderPath, files).ConfigureAwait(false);
                    }
                }
                ...
                else
                {
                    ...
                    if (AddTemp is not null)
                    {
                        try
                        {
                            if (FileService.IsFileExists(MachineFileFolderPath, AddTemp.FileName) == false)
                            {
                                // 파일을 저장하는 로직
                                await FileService.AddResizeImageFile(AddTemp.FileName, MachineFileFolderPath, files).ConfigureAwait(false);
                            }
                        }
                        catch (Exception ex)
                        {
                    ...
                    if (!String.IsNullOrWhiteSpace(RemoveTemp))
                    {
                        try
                        {
                            FileService.DeleteImageFile(MachineFileFolderPath, RemoveTemp);
                        }
                        catch (Exception ex)
                        {
        ...
        public async Task<ResponseModel<bool?>> DeleteMachineFacilityService(List<int> delIdx)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
