    ...
    public class MachineFacilityService : IMachineFacilityService
    {
        ...
        public async ValueTask<ResponseUnit<FacilityDTO>> AddMachineFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                if (result is not null)
                {
                    ...
                    {
                        // 파일 넣기
                        bool? AddFile = await FileService.AddResizeImageFile(NewFileName, MachineFileFolderPath, files);
                    }
        ...
        public async ValueTask<ResponseList<FacilityListDTO>> GetMachineFacilityListService(HttpContext context)
        {
            try
            {
                ...
                List<FacilityListDTO>? model = await FacilityInfoRepository.GetPlaceMachineFacilityList(Convert.ToInt32(placeidx));
        ...
        public async ValueTask<ResponseUnit<FacilityDetailDTO>> GetMachineDetailFacilityService(HttpContext context, int facilityId)
        {
            try
            {
                ...
                if(!String.IsNullOrWhiteSpace(model.Image))
                {
                    ...
                    dto.Image = await FileService.GetImageFile(MachineFileFolderPath, model.Image);
                }
        ...
        public async ValueTask<ResponseUnit<bool?>> UpdateMachineFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                {
                    ImageBytes = await FileService.GetImageFile(MachineFileFolderPath, deleteFileName);
                }
                ...
                if (files is not null)
                {
                    if (String.IsNullOrWhiteSpace(model.Image) || files.FileName != model.Image)
                    {
                        ...
                        await FileService.AddResizeImageFile(model.Image!, MachineFileFolderPath, files);
                    }
                }
                ...
                else
                {
                    ...
                    if (AddTemp is not null)
                    {
                        try
                        {
                            ...
                            {
                                // 파일을 저장하는 로직
                                await FileService.AddResizeImageFile(AddTemp.FileName, MachineFileFolderPath, files);
                            }
                        }
                        catch (Exception ex)
                        {
