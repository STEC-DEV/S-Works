    ...
    public class ConstructFacilityService : IConstructFacilityService
    {
        ...
        public async ValueTask<ResponseUnit<FacilityDTO>> AddConstructFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                if (result is not null)
                {
                    ...
                    {
                        // 파일넣기
                        bool? AddFile = await FileService.AddResizeImageFile(NewFileName, ConstructFileFolderPath, files);
                    }
        ...
        public async ValueTask<ResponseUnit<FacilityDetailDTO>> GetConstructDetailFacilityService(HttpContext context, int facilityId)
        {
            try
            {
                ...
                if(!String.IsNullOrWhiteSpace(model.Image))
                {
                    ...
                    dto.Image = await FileService.GetImageFile(ConstructFileFolderPath, model.Image); // 이미지 byte[]
                }
        ...
        public async ValueTask<ResponseUnit<bool?>> UpdateConstructFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                {
                    ImageBytes = await FileService.GetImageFile(ConstructFileFolderPath, deleteFileName);
                }
                ...
                if(files is not null)
                {
                    if(String.IsNullOrWhiteSpace(model.Image) || files.FileName != model.Image)
                    {
                        ...
                        await FileService.AddResizeImageFile(model.Image!, ConstructFileFolderPath, files);
                    }
                }
                ...
                else
                {
                    ...
                    if(AddTemp is not null)
                    {
                        try
                        {
                            ...
                            {
                                // 파일을 저장하는 로직
                                await FileService.AddResizeImageFile(AddTemp.FileName, ConstructFileFolderPath, files);
                            }
                        }
                        catch(Exception ex)
                        {
