        ...
        public async ValueTask<ResponseUnit<FacilityDTO>> AddConstructFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                if (result is not null)
                {
                    ...
                    {
                        // 파일넣기
                        bool? AddFile = await FileService.AddImageFile(NewFileName, ConstructFileFolderPath, files);
                    }
            ...
            try
            {
                ...
                if (model is not null)
                {
                    ...
                    if (room is not null)
                    {
                        ...
                        {
                            dto.Image = await FileService.GetImageFile(ConstructFileFolderPath, model.Image);
                        }
        ...
        public async ValueTask<ResponseUnit<bool?>> UpdateConstructFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                if (model is not null)
                {
                    ...
                    {
                        if(files is not null) // 파일명이 공백이 아닌 경우
                        {
                            if(!String.IsNullOrWhiteSpace(model.Image))
                            {
                                // 파일넣기
                            ...
                            {
                                // 파일삭제
                                bool DeleteFile = FileService.DeleteImageFile(ConstructFileFolderPath, deleteFileName);
                            }
                        }
                        else // 파일이 공백인 경우
                        {
