...
namespace FamTec.Server.Services.Facility.Type.Contstruct
{
    public class ConstructFacilityService : IConstructFacilityService
    {
        ...
        public async ValueTask<ResponseUnit<AddFacilityDTO>?> AddConstructFacilityService(HttpContext? context, AddFacilityDTO? dto, IFormFile? files)
        {
            try
            {
                ...
                if (files is not null)
                {
                    ...
                    else
                    {
                        ...
                        string? newFilePath = Path.Combine(ConstructFileFolderPath, newFileName);
        ...
        public async ValueTask<ResponseList<FacilityListDTO>?> GetConstructFacilityListService(HttpContext? context)
        {
            try
            {
                ...
                List<FacilityListDTO>? model = await FacilityInfoRepository.GetPlaceConstructFacilityList(Convert.ToInt32(placeidx));
                ...
                if (model is not null)
                {
                    ...
                    if (room is not null)
                    {
                        ...
                        if (!String.IsNullOrWhiteSpace(Image))
                        {
                            ...
                            string[] FileList = Directory.GetFiles(ConstructFileFolderPath);
                            if (FileList is [_, ..])
                            {
        ...
        public async ValueTask<ResponseUnit<bool?>> UpdateConstructFacilityService(HttpContext? context, UpdateFacilityDTO? dto, IFormFile? files)
        {
            try
            {
                string? FileName = String.Empty;
                ...
                if (model is not null)
                {
                    ...
                    if (files is not null)
                    {
                        ...
                        if (di.Exists)
                        {
                            ...
                            {
                                FileName = String.Format("{0}\\{1}", ConstructFileFolderPath, filePath);
        ...
        public async ValueTask<ResponseUnit<int?>> DeleteConstructFacilityService(HttpContext? context, List<int> delIdx)
        {
            try
            {
                int delCount = 0;
