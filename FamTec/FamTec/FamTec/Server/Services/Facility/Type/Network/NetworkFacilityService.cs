...
namespace FamTec.Server.Services.Facility.Type.Network
{
    public class NetworkFacilityService : INetworkFacilityService
    {
        ...
        private readonly ConsoleLogService<NetworkFacilityService> CreateBuilderLogger; /* 콘솔로그 */
        private DirectoryInfo? di; /* 디렉터리 객체 */
        private string? NetworkFileFolderPath; /* 디렉터리 경로 */

        public NetworkFacilityService(IFacilityInfoRepository _facilityinforepository,
        ...
        public async Task<byte[]?> DownloadNetworkFacilityForm()
        {
            try
            {
                ...
                // 두 번째 시트 생성
                var worksheet2 = workbook.Worksheets.Add("통신설비정보");
        ...
        /// <returns></returns>
        public async Task<ResponseModel<bool>> ImportNetworkFacilityService(IFormFile? file)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    using (var workbook = new XLWorkbook(stream))
                    {
                        ...
                        return AddResult switch
                        {
                            ...
                            false => new ResponseModel<bool>() { message = "잘못된 요청입니다.", data = false, code = 404 }
                        };
                    }
                }
            }
        ...
        public async Task<ResponseModel<FacilityDTO>> AddNetworkFacilityService(FacilityDTO dto, IFormFile? files)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                if (result is not null)
                {
                    ...
                    {
                        // 파일 넣기
                        bool? AddFile = await FileService.AddResizeImageFile(NewFileName, NetworkFileFolderPath, files).ConfigureAwait(false);
                    }

                    return new ResponseModel<FacilityDTO>()
                    {
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        public async Task<ResponseModel<List<FacilityListDTO>>> GetNetworkFacilityListService()
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                List<FacilityListDTO>? model = await FacilityInfoRepository.GetPlaceNetworkFacilityList(Convert.ToInt32(placeidx)).ConfigureAwait(false);
        ...
        public async Task<ResponseModel<FacilityDetailDTO>> GetNetworkDetailFacilityService(int facilityId, bool isMobile)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                if (isMobile)
                {
                    ...
                    {
                        byte[]? ImageBytes = await FileService.GetImageFile(NetworkFileFolderPath, model.Image).ConfigureAwait(false);
        ...
        public async Task<ResponseModel<bool?>> UpdateNetworkFacilityService(FacilityDTO dto, IFormFile? files)
        {
            try
            {
                // 파일처리 준비
                ...
                {
                    ImageBytes = await FileService.GetImageFile(NetworkFileFolderPath, deleteFileName).ConfigureAwait(false);
                }
                ...
                if (!String.IsNullOrWhiteSpace(deleteFileName))
                {
                    FileService.DeleteImageFile(NetworkFileFolderPath, deleteFileName);
                }
                ...
                if (files is not null)
                {
                    if (String.IsNullOrWhiteSpace(model.Image) || files.FileName != model.Image)
                    {
                        ...
                        await FileService.AddResizeImageFile(model.Image!, NetworkFileFolderPath, files).ConfigureAwait(false);
                    }
                }
                ...
                else
                {
                    ...
                    if (AddTemp is not null)
                    {
                        try
                        {
                            if (FileService.IsFileExists(NetworkFileFolderPath, AddTemp.FileName) == false)
                            {
                                // 파일을 저장하는 로직
                                await FileService.AddResizeImageFile(AddTemp.FileName, NetworkFileFolderPath, files).ConfigureAwait(false);
                            }
                        }
                        catch (Exception ex)
                        {
                    ...
                    if (!String.IsNullOrWhiteSpace(RemoveTemp))
                    {
                        try
                        {
                            FileService.DeleteImageFile(NetworkFileFolderPath, RemoveTemp);
                        }
                        catch (Exception ex)
                        {
        ...
        public async Task<ResponseModel<bool?>> DeleteNetworkFacilityService(List<int> delIdx)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
