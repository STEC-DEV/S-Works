    ...
    public class NetworkFacilityService : INetworkFacilityService
    {
        ...
        public async ValueTask<ResponseUnit<FacilityDetailDTO>> GetNetworkDetailFacilityService(HttpContext context, int facilityId)
        {
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(model.Image))
                {
                    ...
                    dto.Image = await FileService.GetImageFile(NetworkFileFolderPath, model.Image); // 이미지 Byte[]
                }
        ...
        public async ValueTask<ResponseUnit<bool?>> UpdateNetworkFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                {
                    ImageBytes = await FileService.GetImageFile(NetworkFileFolderPath, deleteFileName);
                }
                ...
                if (files is not null)
                {
                    if (String.IsNullOrWhiteSpace(model.Image) || files.FileName != model.Image)
                    {
                        ...
                        await FileService.AddResizeImageFile(model.Image!, NetworkFileFolderPath, files);
                    }
                }
                ...
                else
                {
                    ...
                    if (AddTemp is not null)
                    {
                        try
                        {
                            ...
                            {
                                // 파일을 저장하는 로직
                                await FileService.AddResizeImageFile(AddTemp.FileName, NetworkFileFolderPath, files);
                            }
                        }
                        catch (Exception ex)
                        {
