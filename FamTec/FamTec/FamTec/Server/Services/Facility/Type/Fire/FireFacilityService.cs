    ...
    public class FireFacilityService : IFireFacilityService
    {
        ...
        public async Task<byte[]?> DownloadFireFacilityForm()
        {
            try
            {
                ...
                // 두 번째 시트 생성
                var worksheet2 = workbook.Worksheets.Add("소방설비정보");
        ...
        /// <returns></returns>
        public async Task<ResponseModel<bool>> ImportFireFacilityService(IFormFile? file)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
        ...
        public async Task<ResponseModel<FacilityDTO>> AddFireFacilityService(FacilityDTO dto, IFormFile? files)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                if (result is not null)
                {
                    ...
                    {
                        // 파일 넣기
                        bool? AddFile = await FileService.AddResizeImageFile(NewFileName, FireFileFolderPath, files).ConfigureAwait(false);
                    }
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        public async Task<ResponseModel<List<FacilityListDTO>>> GetFireFacilityListService()
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                List<FacilityListDTO>? model = await FacilityInfoRepository.GetPlaceFireFacilityList(Convert.ToInt32(placeidx)).ConfigureAwait(false);
        ...
        public async Task<ResponseModel<FacilityDetailDTO>> GetFireDetailFacilityService(int facilityId, bool isMobile)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                if(isMobile)
                {
                    ...
                    {
                        byte[]? ImageBytes = await FileService.GetImageFile(FireFileFolderPath, model.Image).ConfigureAwait(false);
        ...
        public async Task<ResponseModel<bool?>> UpdateFireFacilityService(FacilityDTO dto, IFormFile? files)
        {
            try
            {
                // 파일처리 준비
                ...
                {
                    ImageBytes = await FileService.GetImageFile(FireFileFolderPath, deleteFileName).ConfigureAwait(false);
                }
                ...
                if(!String.IsNullOrWhiteSpace(deleteFileName))
                {
                    FileService.DeleteImageFile(FireFileFolderPath, deleteFileName);
                }
                ...
                if(files is not null)
                {
                    if(String.IsNullOrWhiteSpace(model.Image) || files.FileName != model.Image)
                    {
                        ...
                        await FileService.AddResizeImageFile(model.Image!, FireFileFolderPath, files).ConfigureAwait(false);
                    }
                }
                ...
                else
                {
                    ...
                    if(AddTemp is not null)
                    {
                        try
                        {
                            if(FileService.IsFileExists(FireFileFolderPath, AddTemp.FileName) == false)
                            {
                                // 파일을 저장하는 로직
                                await FileService.AddResizeImageFile(AddTemp.FileName, FireFileFolderPath, files).ConfigureAwait(false);
                            }
                        }
                        catch(Exception ex)
                        {
                    ...
                    if(!String.IsNullOrWhiteSpace(RemoveTemp))
                    {
                        try
                        {
                            FileService.DeleteImageFile(FireFileFolderPath, RemoveTemp);
                        }
                        catch(Exception ex)
                        {
        ...
        public async Task<ResponseModel<bool?>> DeleteFireFacilityService(List<int> delIdx)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
