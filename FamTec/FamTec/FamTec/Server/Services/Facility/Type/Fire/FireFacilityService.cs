...
namespace FamTec.Server.Services.Facility.Type.Fire
{
    public class FireFacilityService : IFireFacilityService
    {
        ...
        public async ValueTask<ResponseUnit<AddFacilityDTO>?> AddFireFacilityService(HttpContext? context, AddFacilityDTO? dto, IFormFile? files)
        {
            try
            {
                ...
                if (files is not null)
                {
                    ...
                    else
                    {
                        ...
                        string? newFilePath = Path.Combine(FireFileFolderPath, newFileName);
                ...
                if (model is not null)
                {
                    ...
                    if (room is not null)
                    {
                        ...
                        if (!String.IsNullOrWhiteSpace(Image))
                        {
                            ...
                            string[] FileList = Directory.GetFiles(FireFileFolderPath);
                            if (FileList is [_, ..])
                            {
        ...
        public async ValueTask<ResponseUnit<bool?>> UpdateFireFacilityService(HttpContext? context, UpdateFacilityDTO? dto, IFormFile? files)
        {
            try
            {
                string? FileName = String.Empty;
                ...
                if (model is not null)
                {
                    ...
                    if (files is not null)
                    {
                        ...
                        if (di.Exists)
                        {
                            ...
                            {
                                FileName = String.Format("{0}\\{1}", FireFileFolderPath, filePath);
        ...
        public async ValueTask<ResponseUnit<int?>> DeleteFireFacilityService(HttpContext? context, List<int> delIdx)
        {
            try
            {
                int delCount = 0;
