    ...
    public class FireFacilityService : IFireFacilityService
    {
        ...
        public async ValueTask<ResponseUnit<FacilityDTO>> AddFireFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                if (result is not null)
                {
                    ...
                    {
                        // 파일 넣기
                        bool? AddFile = await FileService.AddResizeImageFile(NewFileName, FireFileFolderPath, files);
                    }
        ...
        public async ValueTask<ResponseUnit<bool?>> UpdateFireFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                {
                    ImageBytes = await FileService.GetImageFile(FireFileFolderPath, deleteFileName);
                }
                ...
                if(files is not null)
                {
                    if(String.IsNullOrWhiteSpace(model.Image) || files.FileName != model.Image)
                    {
                        ...
                        await FileService.AddResizeImageFile(model.Image!, FireFileFolderPath, files);
                    }
                }
                ...
                else
                {
                    ...
                    if(AddTemp is not null)
                    {
                        try
                        {
                            ...
                            {
                                // 파일을 저장하는 로직
                                await FileService.AddResizeImageFile(AddTemp.FileName, FireFileFolderPath, files);
                            }
                        }
                        catch(Exception ex)
                        {
