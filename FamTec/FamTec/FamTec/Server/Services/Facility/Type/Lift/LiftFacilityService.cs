...
namespace FamTec.Server.Services.Facility.Type.Lift
{
    public class LiftFacilityService : ILiftFacilityService
    {
        ...
        /// <returns></returns>
        public async Task<ResponseModel<bool>> ImportLiftFacilityService(IFormFile? file)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
        ...
        /// <returns></returns>
        public async Task<ResponseModel<FacilityDTO>> AddLiftFacilityService(FacilityDTO dto, IFormFile? files)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                if (result is not null)
                {
                    ...
                    {
                        // 파일 넣기
                        bool? AddFile = await FileService.AddResizeImageFile(NewFileName, LiftFileFolderPath, files).ConfigureAwait(false);
                    }
        ...
        /// <summary>
        /// 사업장에 등록되어있는 승강설비 List 반환
        /// </summary>
        /// <param name="context"></param>
        /// <returns></returns>
        public async Task<ResponseModel<List<FacilityListDTO>>> GetLiftFacilityListService()
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                List<FacilityListDTO>? model = await FacilityInfoRepository.GetPlaceLiftFacilityList(Convert.ToInt32(placeidx)).ConfigureAwait(false);
        ...
        public async Task<ResponseModel<FacilityDetailDTO>> GetLiftDetailFacilityService(int facilityId, bool isMobile)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                if(isMobile)
                {
                    ...
                    {
                        byte[]? ImageBytes = await FileService.GetImageFile(LiftFileFolderPath, model.Image).ConfigureAwait(false);
        ...
        public async Task<ResponseModel<bool?>> UpdateLiftFacilityService(FacilityDTO dto, IFormFile? files)
        {
            try
            {
                // 파일처리 준비
                ...
                {
                    ImageBytes = await FileService.GetImageFile(LiftFileFolderPath, deleteFileName).ConfigureAwait(false);
                }
                ...
                if(!String.IsNullOrWhiteSpace(deleteFileName))
                {
                    FileService.DeleteImageFile(LiftFileFolderPath, deleteFileName);
                }
                ...
                if(files is not null)
                {
                    if(String.IsNullOrWhiteSpace(model.Image) || files.FileName != model.Image)
                    {
                        ...
                        await FileService.AddResizeImageFile(model.Image!, LiftFileFolderPath, files).ConfigureAwait(false);
                    }
                }
                ...
                else
                {
                    ...
                    if(AddTemp is not null)
                    {
                        try
                        {
                            if(FileService.IsFileExists(LiftFileFolderPath, AddTemp.FileName) == false)
                            {
                                // 파일을 저장하는 로직
                                await FileService.AddResizeImageFile(AddTemp.FileName, LiftFileFolderPath, files).ConfigureAwait(false);
                            }
                        }
                        catch(Exception ex)
                        {
                    ...
                    if(!String.IsNullOrWhiteSpace(RemoveTemp))
                    {
                        try
                        {
                            FileService.DeleteImageFile(LiftFileFolderPath, RemoveTemp);
                        }
                        catch(Exception ex)
                        {
        ...
        public async Task<ResponseModel<bool?>> DeleteLiftFacilityService(List<int> delIdx)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
