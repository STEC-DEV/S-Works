        ...
        public async ValueTask<ResponseUnit<FacilityDTO>> AddLiftFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                if (result is not null)
                {
                    ...
                    {
                        // 파일 넣기
                        bool? AddFile = await FileService.AddImageFile(NewFileName, LiftFileFolderPath, files);
                    }
        ...
        public async ValueTask<ResponseUnit<bool?>> UpdateLiftFacilityService(HttpContext context, FacilityDTO dto, IFormFile files)
        {
            try
            {
                ...
                if (model is not null)
                {
                    ...
                    {
                        if(!String.IsNullOrWhiteSpace(model.Image)) // DB에 파일이 있는ㄴ 경우
                        {
                    ...
                    if (updateBuilding == true)
                    {
                        if(files is not null) // 파일이 공백이 아닌경우
                        {
                            ...
                            {
                                // 파일 삭제
                                bool DeleteFile = FileService.DeleteImageFile(LiftFileFolderPath, deleteFileName);
                            }
                        }
                        else // 파일이 공백인 경우
                        {
