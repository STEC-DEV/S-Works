    ...
    public class LiftFacilityService : ILiftFacilityService
    {
        ...
        public async ValueTask<ResponseUnit<FacilityDTO>> AddLiftFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                if (result is not null)
                {
                    ...
                    {
                        // 파일 넣기
                        bool? AddFile = await FileService.AddResizeImageFile(NewFileName, LiftFileFolderPath, files);
                    }
        ...
        public async ValueTask<ResponseUnit<FacilityDetailDTO>> GetLiftDetailFacilityService(HttpContext context, int facilityId)
        {
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(model.Image))
                {
                    ...
                    dto.Image = await FileService.GetImageFile(LiftFileFolderPath, model.Image);
                }
        ...
        public async ValueTask<ResponseUnit<bool?>> UpdateLiftFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                {
                    ImageBytes = await FileService.GetImageFile(LiftFileFolderPath, deleteFileName);
                }
                ...
                if(files is not null)
                {
                    if(String.IsNullOrWhiteSpace(model.Image) || files.FileName != model.Image)
                    {
                        ...
                        await FileService.AddResizeImageFile(model.Image!, LiftFileFolderPath, files);
                    }
                }
                ...
                else
                {
                    ...
                    if(AddTemp is not null)
                    {
                        try
                        {
                            ...
                            {
                                // 파일을 저장하는 로직
                                await FileService.AddResizeImageFile(AddTemp.FileName, LiftFileFolderPath, files);
                            }
                        }
                        catch(Exception ex)
                        {
