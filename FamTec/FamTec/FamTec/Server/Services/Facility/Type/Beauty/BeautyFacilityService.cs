...
namespace FamTec.Server.Services.Facility.Type.Beauty
{
    public class BeautyFacilityService : IBeautyFacilityService
    {
        ...
        public async ValueTask<ResponseUnit<AddFacilityDTO>?> AddBeautyFacilityService(HttpContext? context, AddFacilityDTO? dto, IFormFile? files)
        {
            try
            {
                ...
                if (files is not null)
                {
                    ...
                    else
                    {
                        ...
                        string? newFilePath = Path.Combine(BeautyFileFolderPath, newFileName);
        ...
        public async ValueTask<ResponseList<FacilityListDTO>?> GetBeautyFacilityListService(HttpContext? context)
        {
            try
            {
                ...
                List<FacilityListDTO>? model = await FacilityInfoRepository.GetPlaceBeautyFacilityList(Convert.ToInt32(placeidx));
                ...
                if (model is not null)
                {
                    ...
                    if (room is not null)
                    {
                        ...
                        if (!String.IsNullOrWhiteSpace(Image))
                        {
                            ...
                            string[] FileList = Directory.GetFiles(BeautyFileFolderPath);
                            if (FileList is [_, ..])
                            {
        ...
        public async ValueTask<ResponseUnit<bool?>> UpdateBeautyFacilityService(HttpContext? context, UpdateFacilityDTO? dto, IFormFile? files)
        {
            try
            {
                string? FileName = String.Empty;
                ...
                if (model is not null)
                {
                    ...
                    if (files is not null)
                    {
                        ...
                        if (di.Exists)
                        {
                            ...
                            {
                                FileName = String.Format("{0}\\{1}", BeautyFileFolderPath, filePath);
