    ...
    public class BeautyFacilityService : IBeautyFacilityService
    {
        ...
        public async ValueTask<ResponseUnit<bool?>> UpdateBeautyFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                {
                    ImageBytes = await FileService.GetImageFile(BeautyFileFolderPath, deleteFileName);
                }
                ...
                if(!String.IsNullOrWhiteSpace(deleteFileName))
                {
                    FileService.DeleteImageFile(BeautyFileFolderPath, deleteFileName);
                }
                ...
                if(files is not null)
                {
                    if(String.IsNullOrWhiteSpace(model.Image) || files.FileName != model.Image)
                    {
                        ...
                        await FileService.AddResizeImageFile(model.Image!, BeautyFileFolderPath, files);
                    }
                }
                ...
                else
                {
                    ...
                    if(AddTemp is not null)
                    {
                        try
                        {
                            if(FileService.IsFileExists(BeautyFileFolderPath, AddTemp.FileName) == false)
                            {
                                // 파일을 저장하는 로직
                                await FileService.AddResizeImageFile(AddTemp.FileName, BeautyFileFolderPath, files);
                            }
                        }
                        catch(Exception ex)
                        {
                    ...
                    if(!String.IsNullOrWhiteSpace(RemoveTemp))
                    {
                        try
                        {
                            FileService.DeleteImageFile(BeautyFileFolderPath, RemoveTemp);
                        }
                        catch(Exception ex)
                        {
