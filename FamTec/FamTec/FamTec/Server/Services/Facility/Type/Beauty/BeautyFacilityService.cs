    ...
    public class BeautyFacilityService : IBeautyFacilityService
    {
        ...
        public async Task<ResponseUnit<bool?>> UpdateBeautyFacilityService(HttpContext context, FacilityDTO dto, IFormFile? files)
        {
            try
            {
                ...
                {
                    ImageBytes = await FileService.GetImageFile(BeautyFileFolderPath, deleteFileName).ConfigureAwait(false);
                }
                ...
                if(files is not null)
                {
                    if(String.IsNullOrWhiteSpace(model.Image) || files.FileName != model.Image)
                    {
                        ...
                        await FileService.AddResizeImageFile(model.Image!, BeautyFileFolderPath, files).ConfigureAwait(false);
                    }
                }
                ...
                else
                {
                    ...
                    if(AddTemp is not null)
                    {
                        try
                        {
                            ...
                            {
                                // 파일을 저장하는 로직
                                await FileService.AddResizeImageFile(AddTemp.FileName, BeautyFileFolderPath, files).ConfigureAwait(false);
                            }
                        }
                        catch(Exception ex)
                        {
