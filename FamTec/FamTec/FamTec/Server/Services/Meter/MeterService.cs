...
namespace FamTec.Server.Services.Meter
{
    public class MeterService : IMeterService
    {
        ...
        /// <returns></returns>
        public async Task<ResponseModel<AddMeterDTO>> AddMeterService(AddMeterDTO dto)
        {
            try
            {
                ...
                if (context is null || dto is null)
                    return new ResponseModel<AddMeterDTO>() { message = "잘못된 요청입니다.", data = null, code = 404 };
                ...
                if(MeterInfo is not null)
                    return new ResponseModel<AddMeterDTO>() { message = "이미 존재하는 검침기명 입니다.", data = null, code = 201 };
                ...
                if (AddMeterResult is not null)
                    return new ResponseModel<AddMeterDTO>() { message = "요청이 정상 처리되었습니다.", data = dto, code = 200 };
                else
                    return new ResponseModel<AddMeterDTO>() { message = "서버에서 요청을 처리하지 못하였습니다.", data = null, code = 500 };
            }
            catch(Exception ex)
            {
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        public async Task<ResponseModel<List<MeterDTO>>> GetAllMeterListService()
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                if (context is null)
                    return new ResponseModel<List<MeterDTO>>() { message = "잘못된 요청입니다.", data = null, code = 404 };
                ...
                if(MeterList is not null && MeterList.Any())
                {
                    ...
                    if(ContractList is not null && ContractList.Any())
                    {
                        ...
                        return new ResponseModel<List<MeterDTO>>() { message = "요청이 정상 처리되었습니다.", data = dto, code = 200 };
                ...
                {
                    return new ResponseModel<List<MeterDTO>>() { message = "요청이 정상 처리되었습니다.", data = new List<MeterDTO>(), code = 200 };
                }
            ...
            {
                LogService.LogMessage(ex.ToString());
                return new ResponseModel<List<MeterDTO>>() { message = "서버에서 요청을 처리하지 못하였습니다.", data = null, code = 500 };
            }
