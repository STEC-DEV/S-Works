...
namespace FamTec.Server.Services.Building.Group
{
    public class BuildingGroupService : IBuildingGroupService
    {
        ...
        /// <returns></returns>
        public async Task<ResponseModel<bool>> AddBuildingGroupService(List<AddGroupDTO> dto)
        {
            try
            {
        ...
        /// <param name="dto"></param>
        /// <returns></returns>
        public async Task<ResponseModel<AddGroupInfoDTO>> AddBuildingGroupInfoService(AddGroupInfoDTO dto)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
        ...
        /// <returns></returns>
        public async Task<ResponseModel<List<GroupListDTO?>>> GetBuildingGroupListService(int buildingId)
        {
            try
            {
                var context = HttpContextAccessor.HttpContext;
                ...
                if (context is null)
                    return new ResponseModel<List<GroupListDTO?>>() { message = "잘못된 요청입니다", data = null, code = 404 };
            ...
            catch(Exception ex)
            {
                ...
                CreateBuilderLogger.ConsoleLog(ex);
#endif
                return new ResponseModel<List<GroupListDTO?>>() { message = "서버에서 요청을 처리하지 못하였습니다.", data = null, code = 500 };
            }
        }
        ...
        public async Task<ResponseModel<bool?>> UpdateGroupNameService(UpdateGroupDTO dto)
        {
            try
            {
                ...
                {
                    true => new ResponseModel<bool?>() { message = "수정이 완료되었습니다.", data = true, code = 200 },
        ...
        public async Task<ResponseModel<bool?>> DeleteGroupService(int groupid)
        {
            try
            {
                ...
                return DeleteResult switch
                {
                    ...
                    false => new ResponseModel<bool?>() { message = "서버에서 요청을 처리하지 못하였습니다.", data = false, code = 500 },
                    _ => new ResponseModel<bool?>() { message = "요청이 잘못되었습니다.", data = null, code = 404 }
                };
            }
            catch (Exception ex)
            {
